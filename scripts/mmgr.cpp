...
#include "mmgr.h"
#pragma warning(disable: 6031 6244 6246)
// ---------------------------------------------------------------------------------------------------------------------------------
...
static	const	unsigned int	paddingSize            = 2048; // An extra 8K per allocation!
#else
static	const	unsigned int	hashBits               = 12;
...
#ifndef CUSTOMLEAKLOGS
	#ifdef SCRIPTS_EXPORTS
		#define MEMORYLOGFILE		"memory_scripts.log";
		#define MEMORYLEAKLOGFILE	"memleak_scripts%d.log";
	#elif SHADERS_EXPORTS
		#define MEMORYLOGFILE		"memory_shaders.log";
		#define MEMORYLEAKLOGFILE	"memleak_shaders%d.log";
	#elif BHS_EXPORTS
		#define MEMORYLOGFILE		"memory_bhs.log";
		#define MEMORYLEAKLOGFILE	"memleak_bhs%d.log";
	#elif SDBEDIT
		#define MEMORYLOGFILE		"memory_sdbedit.log";
		#define MEMORYLEAKLOGFILE	"memleak_sdbedit%d.log";
	#elif PROXYDLL_EXPORTS
		#define MEMORYLOGFILE		"memory_d3d8.log";
		#define MEMORYLEAKLOGFILE	"memleak_d3d8%d.log";
	#else
		#define MEMORYLOGFILE		"memory_unknown.log";
		#define MEMORYLEAKLOGFILE	"memleak_unknown%d.log";
	#endif
#endif
static char *memoryLogFile					= MEMORYLOGFILE;
static const char *memoryLeakLogFile		= MEMORYLEAKLOGFILE;


CRITICAL_SECTION criticalSection;
bool criticalInitialized;
...
// Global variables
// ---------------------------------------------------------------------------------------------------------------------------------
bool memoryPassthrough				= false;
bool memoryDestroyPassthrough		= false;
// ---------------------------------------------------------------------------------------------------------------------------------
...
static	void	doCleanupLogOnFirstRun()
{
	if (cleanupLogOnFirstRun)
	{
		...
		static unsigned long attrib = GetFileAttributes(memoryLogFile);
		if (attrib != 0xFFFFFFFF)
		{
			...
			log("-----------------------------------------------------------------------------------------------");
			log("|   Memory log report for:  %02d/%02d/%04d %02d:%02d:%02d                                                |", tme->tm_mon + 1, tme->tm_mday, tme->tm_year + 1900, tme->tm_hour, tme->tm_min, tme->tm_sec);
			log("-----------------------------------------------------------------------------------------------");
			return;
		}
		remove(memoryLogFile);
...
static	const char	*ownerString(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc)
{
	...
	_snprintf(str, 179, "%s(%05d)::%s", sourceFileStripper(sourceFile), sourceLine, sourceFunc);
	return str;
}
...
static	void	dumpAllocations(FILE *fp)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		{
			char output[260];
			sprintf(output, "%06d 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
...
static	void	dumpLeakReport()
{
	...
	if (stats.totalAllocUnitCount)
	{
		...
		int x = 1;
		do {
			sprintf(filename,memoryLeakLogFile,x);
			x++;
		} while (
	...
	else
	{
		...
		if (reservoirBuffer)
		{
			...
			{
				DeallocMemory(reservoirBuffer[i]);
			}
			//DeallocMemory(reservoirBuffer);
			reservoirBuffer = 0;
...
public:
	...
	~MemStaticTimeTracker() {staticDeinitTime = true; dumpLeakReport();DeleteCriticalSection(&criticalSection);criticalInitialized=false;}
};
...
{
	if (memoryPassthrough) return AllocMemory(reportedSize);
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	...
	try
	{
		...
		if (!reservoir)
		{
			...
			reservoir = (sAllocUnit *) AllocMemory(sizeof(sAllocUnit) * 256);
		...
		if (a > b)
		{
			au->actualAddress = AllocMemory(au->actualSize);
		}
		else
		{
			log("[F] Random faiure");
		...
		au->threadId          = GetCurrentThreadId();
...
void	m_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	...
	try
	{
		...
		if (alwaysLogAll)
		{
			log("[-] ----- %8s of addr 0x%08X thread 0x%08X by %s ", allocationTypes[deallocationType], reinterpret_cast<unsigned int>(const_cast<void *>(reportedAddress)),GetCurrentThreadId(), ownerString(sourceFile, sourceLine, sourceFunc));
		}
		...
		if (reportedAddress)
		{
			...
			DeallocMemory(au->actualAddress);
...
void	m_dumpMemoryReport(const char *filename, const bool overwrite)
{
	...
	dumpAllocations(fp); // Redundant, output to leak log anyways.
